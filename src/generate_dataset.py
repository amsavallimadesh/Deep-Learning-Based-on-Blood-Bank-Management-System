...
# --------------------------------------------------
# Generate base synthetic demand
# --------------------------------------------------
def generate_base_demand(days=365):
    dates = pd.date_range(start="2024-01-01", periods=days, freq="D")

    df = pd.DataFrame({
        "date": dates,
        "rbc": np.random.randint(10, 30, size=days),
        "platelets": np.random.randint(5, 20, size=days),
        "plasma": np.random.randint(8, 25, size=days),
    })

    # 🔑 convert ONCE to float (this fixes everything)
    for col in ["rbc", "platelets", "plasma"]:
        df[col] = df[col].astype("float64")

    return df

# --------------------------------------------------
# Add temporal patterns safely
# --------------------------------------------------
def add_temporal_patterns(df):
    ...
    monsoon_mask = df["date"].dt.month.isin([6, 7, 8, 9])

    demand_cols = ["rbc", "platelets", "plasma"]

    # Weekend → reduced demand
    for col in demand_cols:
        df.loc[weekend_mask, col] = df.loc[weekend_mask, col] * 0.65

    # Monsoon → increased demand
    for col in demand_cols:
        df.loc[monsoon_mask, col] = df.loc[monsoon_mask, col] * 1.20

    # Round at the END
    df[demand_cols] = df[demand_cols].round()
...
if __name__ == "__main__":
    base = generate_base_demand()
    temporal = add_temporal_patterns(base)

    temporal.to_csv("synthetic_blood_demand.csv", index=False)
    print("✅ synthetic_blood_demand.csv created")

    scaled = scale_data(temporal)

    scaled[["date", "rbc"]].to_csv("scaled_rbc_demand.csv", index=False)
    scaled[["date", "platelets"]].to_csv("scaled_platelets_demand.csv", index=False)
    scaled[["date", "plasma"]].to_csv("scaled_plasma_demand.csv", index=False)

    print("✅ scaled demand CSVs created")
