...
def split_dataset(filename='synthetic_blood_demand.csv'):
    ...
    # 🔧 BUG FIX — keep only columns that exist
    rbc_cols = [c for c in rbc_cols if c in df.columns]
    platelet_cols = [c for c in platelet_cols if c in df.columns]
    plasma_cols = [c for c in plasma_cols if c in df.columns]
